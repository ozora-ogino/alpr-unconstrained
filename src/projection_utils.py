...
def find_T_matrix(pts, t_pts):
    ...
    [U, S, V] = np.linalg.svd(A)  # pylint: disable=unused-variable
    H = V[-1, :].reshape((3, 3))
...
def getRectPts(tlx, tly, brx, bry):
    return np.matrix([[tlx, brx, brx, tlx], [tly, tly, bry, bry], [1.0, 1.0, 1.0, 1.0]], dtype=float)


def perspective_transform(wh, angles=np.array([0.0, 0.0, 0.0]), zcop=1000.0, dpp=1000.0):
